...
namespace PNet
{
	...
	PResult Socket::Create()
	{
		...
		handle = socket(
			(ipversion == IPVersion::IPv4) ? AF_INET : AF_INET6, 
	...
	PResult Socket::RecvAll(void* destionation, int numberOfBytes)
	{
		...
		while (totalBytesReceived < numberOfBytes)
		{
			...
			int bytesReceived = 0;
			char* bufferOffset = (char*)destionation + totalBytesReceived;
	...
	PResult Socket::SetBlocking(bool isBlocking)
	{
		...
		// instead ioctlsocket, in linux we use ioctl where we pass the socket handle argument
